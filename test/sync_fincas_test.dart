...
void main() {
  ...
  group('ciclo completo', () {
    ...
    test('editar vuelve a dejarla pendiente y la segunda subida la actualiza',
        () async {
      final productorId = await productorLocal();
    ...
    test('tras un fallo de red la finca sigue pendiente y el reintento la sube',
        () async {
      final productorId = await productorLocal();
      await fincaLocal(productorId);
  ...
  group('descarga', () {
    test('una finca de otro dispositivo entra si su productor ya está aquí',
        () async {
      final productorId = await productorLocal();
      await sync.sincronizar();
  ...
  group('borrado remoto de la finca', () {
    ...
    test('arrastra lotes e hijos, y los deja como recibidos, no pendientes',
        () async {
      api.sembrar(
        'fincas',
    ...
    test('si un hijo tenía cambios locales, se descartan pero queda anotado',
        () async {
