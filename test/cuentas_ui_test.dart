...
void main() {
  ...
  testWidgets('la bienvenida ofrece entrar con la cuenta de Google',
    ...
    // No hay pantalla intermedia ni formulario: Google abre su propia ventana.
    await tester.tap(find.text('Ya tengo cuenta'));
    await asentar(tester);

    expect(api.uid, isNotNull);
    expect((await db.syncDao.sesionActual())!.correo, 'productor@gmail.com');
  ...
  testWidgets('desde el perfil se entra con Google y queda a la vista',
    ...
    // El teléfono ya tenía datos, así que avisa antes de reemplazarlos.
    expect(find.text('¿Entrar con su cuenta?'), findsOneWidget);
    await tester.tap(find.widgetWithText(FilledButton, 'Continuar'));
    await asentar(tester);

    expect(find.text('productor@gmail.com'), findsOneWidget);
    expect(find.text('Sin cuenta'), findsNothing);
    expect(api.correo, 'productor@gmail.com');
