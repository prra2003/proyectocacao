...
void main() {
  ...
  Future<Productor> productorLocal() =>
      db.select(db.productores).getSingle();
  ...
  group('subida', () {
    test('un productor nuevo nace pendiente y se sube sellado por el servidor',
        () async {
    ...
    test('editar vuelve a dejarlo pendiente y la segunda subida lo actualiza',
        () async {
      ...
      expect(api.filasDe('productores').single['nombre_completo'], 'Diego Parra');
      // Sigue siendo la misma fila remota, no una copia.
      expect(api.filasDe('productores'), hasLength(1));
    });

    test('el borrado suave viaja como deleted_at, no como un borrado físico',
        () async {
      final id = await repo.guardarProductor(nombreCompleto: 'Diego');
      await sync.sincronizar();
  ...
  group('descarga', () {
    test('un productor creado en otro dispositivo llega a la base local',
        () async {
    ...
    test('la descarga es incremental: la segunda pasada no vuelve a aplicar',
        () async {
    ...
    test('repetir la sincronización no duplica ni deja pendientes', () async {
      ...
      expect(
        filas.every((f) => f.syncStatus == SyncStatus.synced),
        isTrue,
      );
      expect(api.filasDe('productores'), hasLength(2));
    });

    test('trae todas las páginas cuando hay más filas que el tamaño de página',
        () async {
      for (var i = 1; i <= 3; i++) {
  ...
  group('errores', () {
    test('si la API falla, el registro sigue pendiente y se sube al reintentar',
        () async {
      await repo.guardarProductor(nombreCompleto: 'Diego');
