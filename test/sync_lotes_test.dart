...
void main() {
  ...
  group('ciclo completo', () {
    test(
      'crear: nace pendiente y sube sellado, después del productor y la finca',
      () async {
        await loteLocal();
        ...
        final selloFinca =
            api.filasDe('fincas').single['updated_at']! as String;
    ...
    test(
      'editar y volver a sincronizar actualiza la misma fila remota',
      () async {
        final loteId = await loteLocal();
        await sync.sincronizar();
  ...
  group('descarga y dependencias', () {
    ...
    test(
      'un lote que llega antes que su finca se aplaza y no mueve el cursor',
      () async {
    ...
    test(
      'cuando la finca llega, el lote aplazado se aplica solo y sin duplicar',
      () async {
    ...
    test(
      'un lote vivo cuya finca ya está borrada entra muerto, sin revivirla',
      () async {
        await sync.sincronizar();
    ...
    test(
      'varias filas con el mismo updated_at se bajan todas, paginando de a una',
      () async {
        await sync.sincronizar();
  ...
  group('cascadas remotas', () {
    test(
      'Caso A: finca borrada en el servidor arrastra lote y registros',
      () async {
    ...
    test(
      'Caso B: lote borrado en el servidor arrastra sus registros',
      () async {
    ...
    test(
      'Caso C: registros locales pendientes se descartan pero se anotan',
      () async {
        final loteId = await arbolSincronizado();
    ...
    test(
      'lote local pendiente + finca borrada remotamente: manda el borrado',
      () async {
        final loteId = await arbolSincronizado();
        await perfil.guardarLote(
          id: loteId,
